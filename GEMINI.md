...
*   **[Testing Guidelines](docs/testing-guidelines.md)**: Comprehensive testing strategy, test pyramid, coverage requirements, and best practices for the code review agent project.
...
*   **Environment Setup** (in `.env` file):
    ...
    GOOGLE_CLOUD_PROJECT=bpc_askgreg-nonprod
...
*   **Critical Agent Engine Requirements:**
    *   **Never call parent `set_up()` in Agent Engine apps** - Calling `super().set_up()` in `AgentEngineApp` triggers unnecessary Resource Manager API calls that cause deployment failures
    *   **Verify requirements.txt includes `google-cloud-aiplatform`** - The `--no-annotate` flag can accidentally filter this out. Use: `uv export --no-hashes --no-header --no-dev --no-emit-project | sed '/^[[:space:]]*#/d' | sed '/^[[:space:]]*$/d' | sed '/^Resolved/d'`
    *   **Always check Agent Engine logs on deployment failure** - The error message points to logs, which show the actual issue (missing deps, import errors, API permission issues)
...
*   **Deployment Best Practices:**
    *   **Always run deployments without output redirection** to see errors as they happen. Never pipe output to `tail`, `head`, or redirect stderr to `/dev/null` during deployment.
    *   Example: `uv run -m app.app_utils.deploy --location=europe-west1` (not `... 2>&1 | tail -50`)
    *   **Pre-deployment validation:** Run `make test-deploy` before deploying to catch issues early
    *   **Check deployment logs on failure:** Agent Engine logs show the actual error: `gcloud logging read 'resource.type="aiplatform.googleapis.com/ReasoningEngine" AND resource.labels.reasoning_engine_id="YOUR_ID" AND severity>=ERROR' --limit=5 --project=YOUR_PROJECT`
